...
```{r setup, echo=FALSE}
knitr::opts_chunk$set(eval = T, include  = TRUE, warning=FALSE)
```
...
- 4a) Answer: Based on this plot, it seems to be the general case that the number of Starbucks is proportional to the population of a state with a few outliers. 
...
- 4b) Answer: The distributions are very similar (both appear to be Normal) but the caloric distribution for food seems to have a much wider variance.
...
- 4c) Answer: The most common words have to do with drinks, mainly "iced", "tazo", and "bottled". These words apply to many differnt things on the menu, not just one item in particular.
...
- 5a) Answer: Food tends to have more calories but beverages seem to have more carbs per calorie in them.
...
This scatterplot has the same trends as before, where drinks have much less calories on average but have much more grams of carbs per calorie than food does on average. 
...
- 6) Answer: Items with the words "protein" and "tea" seem to have the most calories. Items with the words protein and tea also seem to have the most grams of protein in them.
...
- 7) Answer: We can see that all 3 of these nutrition values are positively correlated in some way. The all increase together with the exception of a few drinks that contain no protein. 
...
- 8) Answer: The two maps show similarities in that states with higher populations also tend to have more Starbucks locations, as seen in California, Texas, and Florida, which are darkly shaded in both maps. However, there are key differences. While high-population states generally have more Starbucks stores, the relationship is not perfectly proportional. Some densely populated states like New York have relatively fewer stores compared to their population, while less populated states like Washington (home to Starbucks' headquarters) have a high concentration of stores relative to their population size.
